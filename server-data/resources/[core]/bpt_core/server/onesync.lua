...
function BPT.OneSync.GetPlayersInArea(source, maxDistance, ignore)
...
---@param maxDistance number
---@param ignore table playerIds to ignore, where the key is playerId and value is true
function BPT.OneSync.GetClosestPlayer(source, maxDistance, ignore)
...
---@param cb function
function BPT.OneSync.SpawnVehicle(model, coords, heading, Properties, cb)
	local veh_model = type(model) == 'string' and joaat(model) or model
	Properties = Properties or {}
	local vector = type(coords) == "vector3" and coords or vec(coords.x, coords.y, coords.z)
	TriggerClientEvent("bpt:requestModel", -1, model)
	CreateThread(function()
		local xPlayer = BPT.OneSync.GetClosestPlayer(vector, 300)
		BPT.GetVehicleType(veh_model, xPlayer.id, function(Type)
			if Type then
...
---@param heading number
---@param cb function
function BPT.OneSync.SpawnObject(model, coords, heading, cb)
...
---@param heading number
---@param cb function
function BPT.OneSync.SpawnPed(model, coords, heading, cb)
    if type(model) == 'string' then model = joaat(model) end
    CreateThread(function()
...
---@param seat number
---@param cb function
function BPT.OneSync.SpawnPedInVehicle(model, vehicle, seat, cb)
    if type(model) == 'string' then model = joaat(model) end
    CreateThread(function()
...
---@return table
function BPT.OneSync.GetPedsInArea(coords, maxDistance, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return table
function BPT.OneSync.GetObjectsInArea(coords, maxDistance, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return table
function BPT.OneSync.GetVehiclesInArea(coords, maxDistance, modelFilter)
...
function BPT.OneSync.GetClosestPed(coords, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vector3 coords
function BPT.OneSync.GetClosestObject(coords, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vector3 coords
function BPT.OneSync.GetClosestVehicle(coords, modelFilter)
	return getClosestEntity(GetAllVehicles(), coords, modelFilter)
end

BPT.RegisterServerCallback("bpt:Onesync:SpawnObject", function(_, cb, model, coords, heading)
    BPT.OneSync.SpawnObject(model, coords, heading, cb)
end)