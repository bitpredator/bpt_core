...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata)
	...
	function self.setAccountMoney(accountName, money, reason)
		...
		if money >= 0 then
			...
			if account then
				money = account.round and BPT.Math.Round(money) or money
				self.accounts[account.index].money = money

				self.triggerEvent('bpt:setAccountMoney', account)
				TriggerEvent('bpt:setAccountMoney', self.source, accountName, money, reason)
			else
	...
	function self.addAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				self.triggerEvent('bpt:setAccountMoney', account)
				TriggerEvent('bpt:addAccountMoney', self.source, accountName, money, reason)
			else
	...
	function self.removeAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				self.triggerEvent('bpt:setAccountMoney', account)
				TriggerEvent('bpt:removeAccountMoney', self.source, accountName, money, reason)
			else
				print(('[^1ERROR^7] Tried To Set Add To Invalid Account ^5%s^0 For Player ^5%s^0!'):format(accountName, self.playerId))
			end
		else
	...
	function self.addInventoryItem(itemName, count)
		...
		if item then
			count = BPT.Math.Round(count)
			item.count = item.count + count
			self.weight = self.weight + (item.weight * count)

			TriggerEvent('bpt:onAddInventoryItem', self.source, item.name, item.count)
			self.triggerEvent('bpt:addInventoryItem', item.name, item.count)
		end
	end
		...
		if item then
			...
			if count > 0 then
				...
				if newCount >= 0 then
					...
					TriggerEvent('bpt:onRemoveInventoryItem', self.source, item.name, item.count)
					self.triggerEvent('bpt:removeInventoryItem', item.name, item.count)
				end
			else
	...
	function self.canCarryItem(itemName, count)
		if BPT.Items[itemName] then
			local currentWeight, itemWeight = self.weight, BPT.Items[itemName].weight
	...
	function self.canSwapItem(firstItem, firstItemCount, testItem, testItemCount)
		...
		if firstItemObject.count >= firstItemCount then
			local weightWithoutFirstItem = BPT.Math.Round(self.weight - (firstItemObject.weight * firstItemCount))
			local weightWithTestItem = BPT.Math.Round(weightWithoutFirstItem + (testItemObject.weight * testItemCount))
	...
	function self.setMaxWeight(newWeight)
		...
		self.triggerEvent('bpt:setMaxWeight', self.maxWeight)
	end
	...
	function self.setJob(newJob, grade)
		...
		if BPT.DoesJobExist(newJob, grade) then
			local jobObject, gradeObject = BPT.Jobs[newJob], BPT.Jobs[newJob].grades[grade]
			...
			TriggerEvent('bpt:setJob', self.source, self.job, lastJob)
	...
	function self.addWeapon(weaponName, ammo)
		if not self.hasWeapon(weaponName) then
			local weaponLabel = BPT.GetWeaponLabel(weaponName)
			...
			self.triggerEvent('bpt:addInventoryItem', weaponLabel, false, true)
		end
	end
	...
	function self.addWeaponComponent(weaponName, weaponComponent)
		...
		if weapon then
			local component = BPT.GetWeaponComponent(weaponName, weaponComponent)

			if component then
				if not self.hasWeaponComponent(weaponName, weaponComponent) then
					self.loadout[loadoutNum].components[#self.loadout[loadoutNum].components + 1] = weaponComponent
					local componentHash = BPT.GetWeaponComponent(weaponName, weaponComponent).hash
					GiveWeaponComponentToPed(GetPlayerPed(self.source), joaat(weaponName), componentHash)
					self.triggerEvent('bpt:addInventoryItem', component.label, false, true)
				end
			end
		end
	end
	...
	function self.setWeaponTint(weaponName, weaponTintIndex)
		...
		if weapon then
			local _, weaponObject = BPT.GetWeapon(weaponName)

			if weaponObject.tints and weaponObject.tints[weaponTintIndex] then
				self.loadout[loadoutNum].tintIndex = weaponTintIndex
				self.triggerEvent('bpt:setWeaponTint', weaponName, weaponTintIndex)
				self.triggerEvent('bpt:addInventoryItem', weaponObject.tints[weaponTintIndex], false, true)
			end
		end
	end
	...
	function self.removeWeapon(weaponName)
		...
		if weaponLabel then
			self.triggerEvent('bpt:removeInventoryItem', weaponLabel, false, true)
		end
	end
		...
		if weapon then
			...
			if component then
				if self.hasWeaponComponent(weaponName, weaponComponent) then
					...
					self.triggerEvent('bpt:removeWeaponComponent', weaponName, weaponComponent)
					self.triggerEvent('bpt:removeInventoryItem', component.label, false, true)
				end
			end
		end
	end
	...
	function self.removeWeaponAmmo(weaponName, ammoCount)
		...
		if weapon then
			weapon.ammo = weapon.ammo - ammoCount
			self.triggerEvent('bpt:setWeaponAmmo', weaponName, weapon.ammo)
		end
	end
	...
	function self.showNotification(msg, notifyType, length)
		self.triggerEvent('bpt:showNotification', msg, notifyType, length)
	end

	function self.showAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
		self.triggerEvent('bpt:showAdvancedNotification', sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
	end

	function self.showHelpNotification(msg, thisFrame, beep, duration)
		self.triggerEvent('bpt:showHelpNotification', msg, thisFrame, beep, duration)
	end
