...
local bptmysql = exports.bptmysql
...
local mysql_method_mt = {
	__call = function(self, query, parameters, cb)
		...
		return bptmysql[self.method](nil, query, parameters, cb, GetCurrentResourceName, false)
	end
}
...
for _, method in pairs({
	...
	MySQL[method] = setmetatable({
		...
		await = function(query, parameters)
			...
			return await(bptmysql[method], query, parameters)
		end
	}, mysql_method_mt)
end
...
local function onReady(cb)
	while GetResourceState('bptmysql') ~= 'started' do
		Wait(50)
	end

	bptmysql.awaitConnection()
