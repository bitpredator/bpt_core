BPT.RegisterCommand({'setcoords', 'tp'}, 'admin', function(xPlayer, args)
...
BPT.RegisterCommand('setjob', 'admin', function(_, args, showError)
	if not BPT.DoesJobExist(args.job, args.grade) then
...
local upgrades = Config.SpawnVehMaxUpgrades and
    ...
    BPT.RegisterCommand('car', 'admin', function(xPlayer, args, showError)
	if not xPlayer then
	...
	BPT.OneSync.SpawnVehicle(args.car, playerCoords, playerHeading, upgrades, function(networkId)
		if networkId then
...
BPT.RegisterCommand({ 'cardel', 'dv' }, 'admin', function(xPlayer, args)
	local PedVehicle = GetVehiclePedIsIn(GetPlayerPed(xPlayer.source), false)
	if DoesEntityExist(PedVehicle) then
		DeleteEntity(PedVehicle)
	end
	local Vehicles = BPT.OneSync.GetVehiclesInArea(GetEntityCoords(GetPlayerPed(xPlayer.source)),
...
BPT.RegisterCommand({ 'fix', 'repair' }, 'admin', function(xPlayer, args, showError)
	...
	xTarget.triggerEvent("bpt:repairPedVehicle")
...
BPT.RegisterCommand('setaccountmoney', 'admin', function(_, args, showError)
...
BPT.RegisterCommand('giveaccountmoney', 'admin', function(_, args, showError)
	if not args.playerId.getAccount(args.account) then
		return showError(_U('command_giveaccountmoney_invalid'))
	end
...
BPT.RegisterCommand('removeaccountmoney', 'admin', function(_, args, showError)
	if not args.playerId.getAccount(args.account) then
...
if not Config.OxInventory then
	BPT.RegisterCommand('giveitem', 'admin', function(_, args)
	...
	BPT.RegisterCommand('giveweapon', 'admin', function(_, args, showError)
	...
	BPT.RegisterCommand('giveammo', 'admin', function(_, args, showError)
	...
	BPT.RegisterCommand('giveweaponcomponent', 'admin', function(_, args, showError)
		if args.playerId.hasWeapon(args.weaponName) then
			local component = BPT.GetWeaponComponent(args.weaponName, args.componentName)
...
BPT.RegisterCommand({ 'clear', 'cls' }, 'user', function(xPlayer)
	xPlayer.triggerEvent('chat:clear')
end, false, { help = _U('command_clear') })

BPT.RegisterCommand({ 'clearall', 'clsall' }, 'admin', function()
	TriggerClientEvent('chat:clear', -1)
end, true, { help = _U('command_clearall') })

BPT.RegisterCommand("refreshjobs", 'admin', function()
	BPT.RefreshJobs()
end, true, { help = _U('command_clearall') })

if not Config.OxInventory then
	BPT.RegisterCommand('clearinventory', 'admin', function(_, args)
		for _, v in ipairs(args.playerId.inventory) do
			if v.count > 0 then
				args.playerId.setInventoryItem(v.name, 0)
			end
		end
		TriggerEvent('bpt:playerInventoryCleared', args.playerId)
	end, true, {
	...
	BPT.RegisterCommand('clearloadout', 'admin', function(_, args)
		for i = #args.playerId.loadout, 1, -1 do
			args.playerId.removeWeapon(args.playerId.loadout[i].name)
		end
		TriggerEvent('bpt:playerLoadoutCleared', args.playerId)
	end, true, {
...
BPT.RegisterCommand('setgroup', 'admin', function(xPlayer, args)
...
BPT.RegisterCommand('save', 'admin', function(_, args)
...
BPT.RegisterCommand('tpm', "admin", function(xPlayer)
	xPlayer.triggerEvent("bpt:tpm")
end, true)

BPT.RegisterCommand('goto', "admin", function(xPlayer, args)
...
BPT.RegisterCommand('bring', "admin", function(xPlayer, args)
	local targetCoords = args.playerId.getCoords()
...
BPT.RegisterCommand('kill', "admin", function(_, args)
	args.playerId.triggerEvent("bpt:killPlayer")
end, true, {
...
BPT.RegisterCommand('freeze', "admin", function(_, args)
	args.playerId.triggerEvent('bpt:freezePlayer', "freeze")
end, true, {
...
BPT.RegisterCommand('unfreeze', "admin", function(_, args)
	args.playerId.triggerEvent('bpt:freezePlayer', "unfreeze")
end, true, {
...
BPT.RegisterCommand("noclip", 'admin', function(xPlayer)
	xPlayer.triggerEvent('bpt:noclip')
end, false)

BPT.RegisterCommand('players', "admin", function()
    local xPlayers = BPT.GetExtendedPlayers() -- Returns all xPlayers
