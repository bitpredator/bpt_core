...
if Config.Multichar then
    AddEventHandler('bpt:onPlayerJoined', function(src, char, data)
    while not next(BPT.Jobs) do
      Wait(50)
    end

    if not BPT.Players[src] then
      local identifier = char .. ':' .. BPT.GetIdentifier(src)
      if data then
        createBPTPlayer(identifier, src, data)
      else
        loadBPTPlayer(identifier, src, false)
      end
    end
  end)
else
  RegisterNetEvent('bpt:onPlayerJoined')
  AddEventHandler('bpt:onPlayerJoined', function()
    local _source = source
    while not next(BPT.Jobs) do
      Wait(50)
    end

    if not BPT.Players[_source] then
...
function onPlayerJoined(playerId)
  local identifier = BPT.GetIdentifier(playerId)
  if identifier then
    if BPT.GetPlayerFromIdentifier(identifier) then
      DropPlayer(playerId,
      ...
      if result then
        loadBPTPlayer(identifier, playerId, false)
      else
        createBPTPlayer(identifier, playerId)
      end
    end
  else
    DropPlayer(playerId,
...
function createBPTPlayer(identifier, playerId, data)
  local accounts = {}
  ...
  if not Config.Multichar then
    MySQL.prepare(newPlayer, { json.encode(accounts), identifier, defaultGroup }, function()
        loadBPTPlayer(identifier, playerId, true)
    end)
  else
...
if not Config.Multichar then
  AddEventHandler('playerConnecting', function(_, _, deferrals)
    ...
    if oneSyncState == "off" or oneSyncState == "legacy" then
      return deferrals.done(('[BPT] BPT Requires Onesync Infinity to work. This server currently has Onesync set to: %s'):format(oneSyncState))
    end

    if not Core.DatabaseConnected then
      return deferrals.done(('[BPT] BPT Cannot Connect to your database. Please make sure it is correctly configured in your server.cfg'):format(oneSyncState))
    end
...
function loadBPTPlayer(identifier, playerId, isNew)
  ...
  -- Job
  if not BPT.DoesJobExist(job, grade) then
    job, grade, duty = 'unemployed', '0', false
    print(("[^3WARNING^7] Ignoring invalid job for ^5%s^7 [job: ^5%s^7, grade: ^5%s^7]"):format(identifier, job, grade))
  end

  local jobObject, gradeObject      = BPT.Jobs[job], BPT.Jobs[job].grades[grade]
  ...
  if not Config.OxInventory then
    ...
    for name, item in pairs(BPT.Items) do
  ...
  if not Config.OxInventory then
    if result.loadout and result.loadout ~= '' then
      ...
      for name, weapon in pairs(loadout) do
        local label = BPT.GetWeaponLabel(name)
  ...
  TriggerEvent('bpt:playerLoaded', playerId, xPlayer, isNew)

  xPlayer.triggerEvent('bpt:playerLoaded',
    {
    ...
    if not Config.OxInventory then
     xPlayer.triggerEvent('bpt:createMissingPickups', Core.Pickups)
    else
    exports.ox_inventory:setPlayerInventory(xPlayer, userData.inventory)
  end
  xPlayer.triggerEvent('bpt:registerSuggestions', Core.RegisteredCommands)
  print(('[^2INFO^0] Player ^5"%s"^0 has connected to the server. ID: ^5%s^7'):format(xPlayer.getName(), playerId))
end

AddEventHandler('chatMessage', function(playerId, _, message)
  local xPlayer = BPT.GetPlayerFromId(playerId)
...
AddEventHandler('playerDropped', function(reason)
  ...
  if xPlayer then
    TriggerEvent('bpt:playerDropped', playerId, reason)

    Core.SavePlayer(xPlayer, function()
        BPT.Players[playerId] = nil
    end)
  end
end)

AddEventHandler('bpt:playerLogout', function(playerId, cb)
  local xPlayer = BPT.GetPlayerFromId(playerId)
  if xPlayer then
    TriggerEvent('bpt:playerDropped', playerId)
  ...
  TriggerClientEvent("bpt:onPlayerLogout", playerId)
end)

if not Config.OxInventory then
  RegisterNetEvent('bpt:updateWeaponAmmo')
  AddEventHandler('bpt:updateWeaponAmmo', function(weaponName, ammoCount)
    local xPlayer = BPT.GetPlayerFromId(source)
  ...
  RegisterNetEvent('bpt:giveInventoryItem')
  AddEventHandler('bpt:giveInventoryItem', function(target, type, itemName, itemCount)
    local playerId = source
    local sourceXPlayer = BPT.GetPlayerFromId(playerId)
    local targetXPlayer = BPT.GetPlayerFromId(target)
    ...
    elseif type == 'item_account' then
      if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
        ...
        sourceXPlayer.showNotification(_U('gave_account_money', BPT.Math.GroupDigits(itemCount), Config.Accounts[itemName].label, targetXPlayer.name))
        targetXPlayer.showNotification(_U('received_account_money', BPT.Math.GroupDigits(itemCount), Config.Accounts[itemName].label,
          sourceXPlayer.name))
      else
    ...
    elseif type == 'item_weapon' then
      if sourceXPlayer.hasWeapon(itemName) then
        local weaponLabel = BPT.GetWeaponLabel(itemName)
        if not targetXPlayer.hasWeapon(itemName) then
          local _, weapon = sourceXPlayer.getWeapon(itemName)
          local _, weaponObject = BPT.GetWeapon(itemName)
          itemCount = weapon.ammo
          local weaponComponents = BPT.Table.Clone(weapon.components)
  ...
  RegisterServerEvent('bpt:removeInventoryItem', function(type, itemName, itemCount)
    local playerId = source
    local xPlayer = BPT.GetPlayerFromId(playerId)
    ...
    if type == 'item_standard' then
        if itemCount == nil or itemCount < 1 then
          ...
          if not xItem or itemCount > xItem.count or xItem.count < 1 then
            ...
            BPT.CreatePickup('item_standard', itemName, itemCount, pickupLabel, playerId)
      ...
      elseif type == 'item_account' then
        if itemCount == nil or itemCount < 1 then
          ...
          if not account or itemCount > account.money or account.money < 1 then
            ...
            local pickupLabel = ('%s [%s]'):format(account.label, _U('locale_currency', BPT.Math.GroupDigits(itemCount)))
            BPT.CreatePickup('item_account', itemName, itemCount, pickupLabel, playerId)
            xPlayer.showNotification(_U('threw_account', BPT.Math.GroupDigits(itemCount), string.lower(account.label)))
          end
        end
      elseif type == 'item_weapon' then
      ...
      local _, weaponObject = BPT.GetWeapon(itemName)
      local components = BPT.Table.Clone(weapon.components)
      ...
      BPT.CreatePickup('item_weapon', itemName, weapon.ammo, pickupLabel, playerId, components, weapon.tintIndex)
      end
    end
  end)

  RegisterNetEvent('bpt:useItem')
  AddEventHandler('bpt:useItem', function(itemName)
    local source = source
    local xPlayer = BPT.GetPlayerFromId(source)
    local count = xPlayer.getInventoryItem(itemName).count

    if count > 0 then
        BPT.UseItem(source, itemName)
    else
      xPlayer.showNotification(_U('act_imp'))
    end
  end)

  RegisterNetEvent('bpt:onPickup')
  AddEventHandler('bpt:onPickup', function(pickupId)
    local pickup, xPlayer, success = Core.Pickups[pickupId], BPT.GetPlayerFromId(source)
    ...
    if pickup then
      ...
      if success then
        ...
        TriggerClientEvent('bpt:removePickup', -1, pickupId)
      end
    end
  end)
end

BPT.RegisterServerCallback('bpt:getPlayerData', function(source, cb)
  local xPlayer = BPT.GetPlayerFromId(source)
...
BPT.RegisterServerCallback('bpt:isUserAdmin', function(source, cb)
  cb(Core.IsPlayerAdmin(source))
end)

BPT.RegisterServerCallback('bpt:getOtherPlayerData', function(_, cb, target)
  local xPlayer = BPT.GetPlayerFromId(target)
...
BPT.RegisterServerCallback('bpt:getPlayerNames', function(source, cb, players)
  players[source] = nil
